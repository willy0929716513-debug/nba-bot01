...
# ==========================================
# NBA V79.1 Quantum Clarity - Same Day Sync
# ==========================================
...
HOME_ADV = 2.4
SPREAD_STD = 13.5
DISPLAY_TIME_OFFSET = -10  # 顯示時間偏移（分鐘）
...
def run():
            ...
            for o in m_obj["outcomes"]:
                ...
                if edge > (0.015 if is_locked else EDGE_THRESHOLD):
                    pick = {
                        ...
                        "display_time": display_tw.strftime("%H:%M"), "locked": is_locked, 
    ...
    message = f"🛡️ **NBA V79.1 Quantum Clarity**\n⏱ {now_tw.strftime('%m/%d %H:%M')}\n"
    ...
    if not grouped:
        ...
        for date, picks in grouped.items():
            ...
            day_parlay_candidates = [] # 用於存放同日串關標的
            ...
            for p in picks:
                ...
                # 判定是否具備串關資格 (EV高且CLV未跌)
                is_parlay_ready = (p["ev"] > PARLAY_EV_THRESHOLD and p["clv"] >= 0 and not p["is_live"])
                parlay_tag = "🔗 " if is_parlay_ready else ""
                if is_parlay_ready: day_parlay_candidates.append(p)
                
                message += f"{live_tag}{parlay_tag}**{p['match']}** {p['label']}\n"
                message += f"{'🔒' if p['locked'] else '✨'} ⏰ {p['display_time']} | 🎯 {TEAM_CN.get(p['team'],p['team'])} {p['point']:+} @ **{p['odds']}**\n"
            ...
            # --- 同日串關邏輯 ---
            if len(day_parlay_candidates) >= 2:
                day_parlay_candidates.sort(key=lambda x: x["prob"], reverse=True)
                top = day_parlay_candidates[:2]
