...
# ==========================================
# NBA V170 Sharp Syndicate - $1000 Bankroll
# ==========================================
...
# 資金控管參數 (以 1000 元為基準)
TOTAL_BANKROLL = 1000
KELLY_FRACTION = 0.25 # 採用 1/4 凱利，追求穩健增長
MAX_BET_PER_GAME = TOTAL_BANKROLL * 0.15 # 單場上限 150 元
...
def pace_factor(home, away):
    return abs(PACE.get(home, 99) - PACE.get(away, 99)) * 0.1
...
def run():
    ...
    for g in games:
        ...
        model_spread, pace = predict_spread(home, away), pace_factor(home, away)
            ...
            for m in book["markets"]:
                ...
                for o in m["outcomes"]:
                    ...
                    if ev >= MIN_EV_THRESHOLD and clv >= MIN_CLV:
                        # 凱利公式下注計算
                        b = o["price"] - 1
                        kelly_p = (b * prob - (1 - prob)) / b
                        clv_boost = min(1.2, max(0.8, 1 + (clv - 0.5) * 0.2))
                        suggested_amt = TOTAL_BANKROLL * kelly_p * KELLY_FRACTION * clv_boost
                        final_bet = round(max(0, min(suggested_amt, MAX_BET_PER_GAME)))
                        ...
                        pick = {
                            ...
                            "ev": ev, "prob": prob, "clv": clv, "bet": final_bet,
    ...
    message = f"🛡️ **NBA V170 Sharp Syndicate**\n⏱ {now_tw.strftime('%m/%d %H:%M')}\n"
    message += f"💰 目前本金：**{TOTAL_BANKROLL} 元** | 🚫 過濾: CLV < {MIN_CLV}\n"
