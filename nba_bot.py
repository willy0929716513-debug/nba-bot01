...
# ==========================================
# NBA V420 Sharp Syndicate - Official Final
# ==========================================
...
# 策略與資金參數
MIN_EV_THRESHOLD = 0.15  # 期望值門檻
MIN_CLV = 0.0           # 只要領先市場就考慮
TOTAL_BANKROLL = 1000   # 總本金
KELLY_FRACTION = 0.25   # 凱利係數 (1/4 Kelly)
MAX_SINGLE_BET = 150    # 單注上限
PARLAY_BET_AMOUNT = 50  # 串關固定注碼
...
def run():
    ...
    try:
        ...
        requests.post(WEBHOOK, json={"content": f"❌ **系統報錯**: API 連線失敗 - {str(e)}"})
        return
    ...
    for g in games:
        ...
        # 過濾太久以後或已經結束的比賽 (顯示未來 150 分鐘內的比賽)
    ...
    # 4. 輸出邏輯
    if not output_list:
        diag_msg = f"🔍 **Syndicate 診斷回報** [{now_tw.strftime('%H:%M')}]\n✅ API 正常 (分析了 {analyzed_count} 場比賽)\n⚠️ 無場次符合 EV > {MIN_EV_THRESHOLD}\n💡 結論：莊家盤口目前極為精準，建議觀望。"
        requests.post(WEBHOOK, json={"content": diag_msg})
        return
    ...
    if len(output_list) >= 2:
        ...
        for n in [2, 3]:
            ...
            for combo in combinations(output_list, n):
                ...
                for t in combo:
                    c_odds *= t["odds"]; c_ev += t["ev"]; c_prob *= t["prob"]
                score = (c_ev / n) * 0.6 + c_prob * 0.4
                top_combos.append((score, combo, c_odds))
        
        if top_combos:
            best = max(top_combos, key=lambda x: x[0])
            msg += f"\n💎 **AI 推薦串關 (賠率 {best[2]:.2f})**\n"
            for t in best[1]: msg += f"✅ {t['match']} {t['label']} ({t['team']} {t['point']})\n"
