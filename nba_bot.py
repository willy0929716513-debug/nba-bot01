...
# ==========================================
# NBA V420 Sharp Syndicate - Final
# ==========================================
...
# 【調整區】
MIN_EV_THRESHOLD = 0.05  # 先調低確保有場次出報
MIN_CLV = 0.0           
TOTAL_BANKROLL = 1000   
KELLY_FRACTION = 0.25   
...
def run():
    ...
    if not API_KEY or not WEBHOOK:
        print("Error: Secrets 沒設定好")
        return

    params = {"apiKey": API_KEY, "regions": "us", "markets": "spreads", "oddsFormat": "decimal"}
    r = requests.get(BASE_URL, params=params)
    if r.status_code != 200:
        requests.post(WEBHOOK, json={"content": f"❌ API 失敗: {r.status_code}"})
        return
    ...
    if not output_list:
        requests.post(WEBHOOK, json={"content": f"🔍 診斷：分析了 {len(games)} 場，但無符合 EV > {MIN_EV_THRESHOLD} 的場次。"})
        return

    msg = f"🛡️ **NBA Sharp Syndicate**\n⏱ {now_tw.strftime('%m/%d %H:%M')}\n"
    for p in output_list[:8]: # 限制長度防止 Discord 擋掉
