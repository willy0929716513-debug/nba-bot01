...
# ==========================================
# NBA V91 Quantum Uniform (Spread Only, +受讓 -讓分)
# ==========================================
...
def run():
    ...
    for g in games:
        ...
        for m_obj in books[0]["markets"]:
            ...
            if market_key != "spreads": continue
            for o in m_obj["outcomes"]:
                ...
                prob = sum(1 for _ in range(SIMS) if random.gauss(model_val if o["name"]==home else -model_val, SPREAD_STD) + o["point"] > 0) / SIMS
    ...
    message = f"🛡️ **NBA V91 Quantum Uniform**\n⏱ {now_tw.strftime('%m/%d %H:%M')}\n"
    ...
    if len(parlay_candidates) >= 2:
        ...
        for c in parlay_candidates:
            parlay_dict.setdefault(c['date_key'], []).append(c)
